...
var commands = {
    ...
    whois: (function () {
        ...
        return function (target, room, user) {
            ...
            if (target === 'Scraf') message = 'The creator of this server.';
    ...
    rps: function (target, room, user) {
        ...
        if (rng === target) return this.sendPm('I chose ' + rng + '. The result is a tie!');
        if (rng === options[0]) {
            if (target === options[1]) return this.sendPm('I chose ' + rng + '. You win o3o!');
            if (target === options[2]) return this.sendPm('I chose ' + rng + '. I win and you lose! ;)');
        }
        if (rng === options[1]) {
            if (target === options[2]) return this.sendPm('I chose ' + rng + '. You win o3o!');
            if (target === options[0]) return this.sendPm('I chose ' + rng + '. I win and you lose! ;)');
        }
        if (rng === options[2]) {
            if (target === options[0]) return this.sendPm('I chose ' + rng + '. You win o3o!');
            if (target === options[1]) return this.sendPm('I chose ' + rng + '. I win and you lose! ;)');
        }
    },
